const loadFonts = (document) => {
  ...
  if (sessionStorage.getItem("fontsLoadedWithClass")) { document.documentElement.classList.add("fonts-loaded"); return; }
  ...
  if ("fonts" in document) {
    Promise.all([
      ...
      sessionStorage.setItem("fontsLoadedWithClass", "true");
    });
  }
};
const getPreference = (document) => {
  let theme;
  if (typeof localStorage !== "undefined" && localStorage.getItem("theme")) theme = localStorage.getItem("theme"); else theme = window.matchMedia("(prefers-color-scheme: dark)").matches ? "dark" : "light";
  document.documentElement.setAttribute("data-theme", theme);
};
getPreference(document);
loadFonts(document);
document.addEventListener("astro:before-swap", e => { getPreference(e.newDocument); });
